...
121Q32 is an unmerged Draft candidate for iteration method 1.2.0 and system map 0.2.0. It adds canonical project components, a typed declared propagation topology, deterministic fixpoint closure, component/surface decisions, map delta and residue binding; it also derives the materialized map from registries plus a layout overlay. Until independent exact-HEAD acceptance and merge closeout, method 1.1.0 and map 0.1.0 remain Current. Declared closure is not scientific causal identification or proof that the registries exhaust the project.
