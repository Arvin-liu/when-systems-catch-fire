...
The subsequent consolidated review of PR #130 exact head `f33be64b…` reproduced eleven more public runtime and standard-JSON-Schema bypasses, `R5A-CR-001`–`011`. The single authorized [consolidated contract-bypass narrow repair](./governance/r5a-consolidated-contract-bypass-repair-r1.md) now rejects every exact fixture, binds runtime/schema representations where both exist, validates all six schemas against Draft 2020-12 with valid/invalid instances, and blocks five identity/expectation/bypass mutations. Its lifecycle is `R5A_CONSOLIDATED_CONTRACT_BYPASS_NARROW_REPAIR_DRAFT_AWAITING_REVIEW`: implementation evidence exists, but independent exact-head acceptance has not occurred. PR #130 remains Draft/unmerged/non-Main/non-Current, and no later stage is authorized.
