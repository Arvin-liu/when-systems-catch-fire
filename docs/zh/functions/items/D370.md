...
- 定义域 / Domain: `X_{D370}`
- 值域 / Codomain: `Y_{D370}`
- 数学表达 / Expression: `F_{D370}(x) := M1的容斥交叉相关增强[D342](D342.md)波动累积：交叉相关项Cov∝pᵢpⱼ使总方差增大→波动累积更严重。增强因子∝1+Σᵢ<ⱼpᵢpⱼ/ΣVar(ΔΦᵢ)。"坏消息成群来"不只预测恶化还放大"折腾有害"——双重打击。`
