...
- 定义域 / Domain: `X_{D375}`
- 值域 / Codomain: `Y_{D375}`
- 数学表达 / Expression: `F_{D375}(x) := M6的δ_c不归点恰好是[D305](D305.md)退化路径的分叉点。δ<δ_c在d=4附近振荡（可回复），δ>δ_c沿退化路径滑走。δ_c是"还能弹回来"和"开始滑走"的精确分界——退化路径的启动条件。`
