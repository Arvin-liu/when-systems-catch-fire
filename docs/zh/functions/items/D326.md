...
- 定义域 / Domain: `X_{D326}`
- 值域 / Codomain: `Y_{D326}`
- 数学表达 / Expression: `F_{D326}(x) := M13的正反馈K^k饱和来自p_max的物理下界p_min>0。K^k有效值=K^k/(1+(K^k-1)·p_min/p_max)，K^k·p_min≈p_max时饱和。饱和后稳态p_max≈p_min·K/(K-1)。K越大稳态越低——强正反馈把p_max压到极低，但代价是[D312](D312.md)噪声放大。[D242](D242.md)精度-鲁棒性权衡在正反馈回路中的具体表现。`
