...
- 定义域 / Domain: `X_{D309}`
- 值域 / Codomain: `Y_{D309}`
- 数学表达 / Expression: `F_{D309}(x) := M10的缓冲耗尽后可重建条件：g_eff>g_critical≈√(2ΔΦ/n)。g_eff低于此阈值时，即使p_max大幅下降，耦合强度也不足以重建缓冲——"缓冲不可逆"状态。与[D295](D295.md)正反馈不可逆点形成双重不可逆：[D295](D295.md)是p_max不可逆，[D309](D309.md)是缓冲不可逆。两个不可逆点可以不同时到达——缓冲可能先于p_max进入不可逆。`
