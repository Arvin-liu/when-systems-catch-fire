...
- 定义域 / Domain: `X_{D371}`
- 值域 / Codomain: `Y_{D371}`
- 数学表达 / Expression: `F_{D371}(x) := M2的极小点复活总代价∝Φ(拐点)，与n无关。但[D264](D264.md)启动阈值∝(n-1)/n→n大时启动更难。联合：复活代价本身不随n增长，但启动阈值随n增长→大系统复活更难不是因为代价大而是因为启动难。`
