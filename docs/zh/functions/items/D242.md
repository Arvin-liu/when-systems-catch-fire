...
- 定义域 / Domain: `X_{D242}`
- 值域 / Codomain: `Y_{D242}`
- 数学表达 / Expression: `F_{D242}(x) := [D241](D241.md)四象限中的相变路径： 起点：无意识象限（ι低, P_exit低） - dim=2（1/ln门控，二值判断） - ε_aware=0（不感知退出权信号） - Ψ=0 相变2触发（dim从2跳到3）：`
