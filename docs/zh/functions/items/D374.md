...
- 定义域 / Domain: `X_{D374}`
- 值域 / Codomain: `Y_{D374}`
- 数学表达 / Expression: `F_{D374}(x) := M5的弱容斥窗口内逃逸速度v_escape(弱容斥)=v_escape(耦合主导)·(1-f_excl)^(1/2)。窗口内[D289](D289.md)黄金逃逸点仍有效但效率降低。窗口边界处v_escape→0——[D332](D332.md)不可逆线即逃逸速度归零线。`
