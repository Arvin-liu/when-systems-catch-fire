...
- 定义域 / Domain: `X_{D402}`
- 值域 / Codomain: `Y_{D402}`
- 数学表达 / Expression: `F_{D402}(x) := M5的[D388](D388.md)交点处K=1（确定性不可逆），[D359](D359.md)临界K=√n（统计不可逆）。确定性不可逆先于统计不可逆——K=1时系统确定性地进入不可逆，K=√n时涨落使不可逆变得随机。`
