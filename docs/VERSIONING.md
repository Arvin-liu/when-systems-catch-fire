...
121Q31 提出之元写作法 `0.4.0` Draft 候选：把素材来源扩展为外部输入与点火增量输出，并增加 spec 驱动的完整可点击系统图。该 minor 候选改变 L6 生成接口，但 `0.3.0` 在独立接受、合并和 Current 收口前仍是当前版本。系统图版本独立为 `0.1.0-candidate`；它不改变七层架构、Foundation、迭代操作法 1.1.0 或成果正文所绑定的方法版本。
