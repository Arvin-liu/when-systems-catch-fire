...
def check_against_committed(repo_root: str, target: str = None) -> bool:
    """Return True iff the regenerated projection is byte-identical to the
    committed file. Used by the CI fixed-point gate and the regression suite.

    Fixes D2 (was a self-comparison): the regenerated projection is compared
    against the *committed* file, never against the freshly-written file.
    """
    target = target or os.path.abspath(OUT_PATH)
    committed = None
    if os.path.exists(target):
        with open(target, "r", encoding="utf-8") as fh:
            committed = fh.read()
    new = generate(repo_root, None)  # in-memory only; never overwrites committed file
    buf = io.StringIO()
    json.dump(new, buf, ensure_ascii=False, sort_keys=True, indent=2)
    buf.write("\n")
    return committed is not None and buf.getvalue() == committed
...
def main() -> int:
    ...
    ap.add_argument("--check", action="store_true",
                    help="regenerate in-memory and assert byte-identical to the committed file (does NOT overwrite it)")
    args = ap.parse_args()
    repo = os.path.abspath(args.repo)
    target = args.out or os.path.abspath(OUT_PATH)
    if args.check:
        # D2 fix: compare against the COMMITTED file, captured before any
        # regeneration. A stale committed projection now fails closed here
        # (instead of being masked by self-comparison).
        if not check_against_committed(repo, target):
        ...
        return 0
    # default: (re)generate and write the projection to disk
