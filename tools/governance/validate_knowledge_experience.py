...
def resolve_link(source: Path, target: str) -> Path:
    ...
    try:
        ...
        raise AssertionError(f"link escapes repository: {source.relative_to(ROOT)} -> {target}") from exc
    return resolved
...
def assert_links_and_anchors(paths: list[Path]) -> int:
    ...
    for source in paths:
        ...
        if "<details" in text.lower():
            raise AssertionError(f"important content hidden by details: {source.relative_to(ROOT)}")
        for target, anchor in markdown_links(source):
            ...
            if not destination.exists():
                raise AssertionError(f"broken link: {source.relative_to(ROOT)} -> {target}")
            if anchor:
                if destination.suffix != ".md" or anchor not in explicit_anchors(destination):
                    raise AssertionError(f"broken explicit anchor: {source.relative_to(ROOT)} -> {target}#{anchor}")
            checked += 1
    return checked
...
def reachable_within_two_clicks() -> set[str]:
    ...
    return {path.relative_to(ROOT.resolve()).as_posix() for path in reached}
