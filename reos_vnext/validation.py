...
def _validate_question(question: Any, path: str, issues: list[ValidationIssue]) -> None:
    ...
    if record is None:
        return
    allowed = {"question_ref", "summary", "question_digest", "root_digest", "version", "amendments"}
    _required(record, allowed, path, issues)
    _keys(record, allowed, path, issues)
    ...
    if amendments is None:
        return
    previous = record.get("root_digest")
    for index, amendment in enumerate(amendments):
        ...
        if item.get("to_digest") == item.get("from_digest"):
            _issue(issues, "QUESTION_MUTATION", f"{amendment_path}.to_digest", "amendment must change the question digest")
    ...
    if amendments and previous != record.get("question_digest"):
        _issue(issues, "QUESTION_MUTATION", f"{path}.amendments", "last amendment does not reach current digest")
    if not amendments and record.get("root_digest") != record.get("question_digest"):
        _issue(issues, "QUESTION_MUTATION", f"{path}.root_digest", "unamended question must have equal root/current digest")
