...
- **Full cascade regen done locally (all validations green).** The knowledge-experience regen (commit `12825e8f`) introduced a new tracked file (`KNOWLEDGE/indexes/architecture_governance/part-017.md`) that shifted path-accounting (3752→3753) and the nonfunction-claims outputs, failing `foundation-validation` run `30909223759` (`discovery:every-repository-path-accounted` + `generator:deterministic NONFUNCTION_CLAIM_OUTPUT_DRIFT`). Re-ran the entire foundation chain (path-accounting manifest; function-asset census + deep adjudication; nonfunction-claims adjudication; migration — default generate mode, no `--generate` flag) then self-correction + human-results + knowledge-experience (2 passes) to a consistent fixed point. Local: path-accounting 9/9, `validate_foundation` 63/63, `validate_nonfunction_claim_closure` valid, self-correction OK, human-results OK, knowledge-experience OK + audit OK.
