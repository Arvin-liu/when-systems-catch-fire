...
## Impact-spec re-baseline (Round 0 step 4 — run `30910955942` step 42)

After the full cascade regen (commit `f5677aed`) closed the earlier drift, `foundation-validation` run `30910955942` reached step 42 (`Task 107 — generator determinism & reconciliation staleness (Layer B negative fixtures)`) and failed there — the deepest gate yet, previously masked because earlier runs aborted before reaching it. The earlier steps (1–41: `validate_foundation`, knowledge-experience, self-correction, all `unittest` matrix) all passed.

- **Failure:** `tests/test_propagation_reconciliation.py::TestRemediatedRepository::test_run_check_clean` → `vr.run_check(REPO)` returned 6 problems, all `MACHINE_RECORD_IMPACT` drift for dimensions 104/105/106:
  `declared NO_IMPACT_JUSTIFIED but derived IMPACT_REQUIRED (changed=['RESULTS/CLAIM-DELTA.md','RESULTS/IMPACT-ANALYSIS.md','RESULTS/EVIDENCE-LINEAGE.md'])`.
- **Root cause (evidence-based):** the Round 0 self-correction cascade regen (commit `683a295e`) legitimately rewrote those 3 `RESULTS/*.md` files. They are the governed sources for `MACHINE_RECORD_IMPACT` (`tools/propagation/impact_contract.py:GOVERNED_SOURCES`). The impact specs at `data/operations/propagation/106-impact/{104,105,106}-impact.json` recorded an old `baseline_sha256` of those files; after the rewrite, `compute_dimension` derives `IMPACT_REQUIRED`, which contradicts the `declared NO_IMPACT_JUSTIFIED` → fail-closed drift. This is the reconciliation mechanism working exactly as designed.
- **Fix (minimal, designed remediation):** re-recorded `baseline_sha256` of the 3 changed `RESULTS/*.md` files in the 3 impact-spec files via the tool's own `generate_impact_spec`, preserving each dimension's `declared` decision (`NO_IMPACT_JUSTIFIED`). Diff scope: 3 files × 6 lines = only the 3 changed-file hashes; `RESULTS/SELF-CORRECTION-AUDIT.md` (4th governed source) was untouched, confirming surgical scope. Local: `validate_reconciliation.py --check` → `RECONCILIATION_OK` for impact-spec dimensions (6 problems → 0); all 3 specs `IMPACT_OK` individually.
- **Deliberately NOT touched:** `current-truth-projection.json`. `run_check` rewrites it as a side effect; locally this produced a "projection cites non-terminal task 112/113/114" complaint driven by a **stale local `origin/main` ref** (`cac043d4`, PR #188). CI run `30910955942` passed the projection steps (25/26) and failed step 42 **only** on `MACHINE_RECORD_IMPACT` — the lifecycle-events file contains `TERMINAL_SUCCESS` records for 110–114, so CI's fresh `origin/main` resolves them terminal. The projection is correct in CI and is left untouched.
- Commit: impact-spec re-baseline (3 files) + this checkpoint + ledger entry.

## Round 0 status: CLOSED (REMOTE_VERIFIED)

- **`foundation-validation` run `30914688351` (job `92009703948`, head `f179da5251947818cdba9af85ae30a99b0beadf1`) → `conclusion=success`.** Steps 1–43 all passed, including step 18 (knowledge-experience clone-independence), step 42 (generator determinism & reconciliation staleness — the `MACHINE_RECORD_IMPACT` impact-spec re-baseline), and step 43 (fail-closed propagation reconciliation). `repository-path-accounting-preflight` (run `30903273283`) was already `success`. Both relevant remote workflows are green on the exact child-branch head → **Round 0 closure condition satisfied** per `CONTINUOUS-SUPERVISOR-AMENDMENT.md` (control tip `fe756fc1`).
- Ledger entry `ROUND0_REMOTE_VERIFIED` appended; this checkpoint records the closure.
- **Round 0 is complete.** Rounds 1–9 now begin under the **callable-capability-federation** architecture per `ARCHITECTURE-AMENDMENT-EXTERNAL-CAPABILITY-FEDERATION.md`: executor-neutral capability-adapter boundary, invocation-first / thin-adapter / no source vendoring, approval/watchdog state machine, license intake, OpenAI4S invocation-first mapping, narrow experiment, contradiction-gate fix, executor-substitution pilot, review packet + stacked Draft PR.

## Entering Round 1 (TASK.md §4)

Deliver an explicit executor-neutral boundary with ≥9 fail-closed schemas (`ExecutorCapabilityDeclaration`, `ExecutionPacket`, `ExecutionLease`, `ExecutorObservation`, `ApprovalRequest`, `ExecutionCheckpoint`, `ResumeCapsule`, `ExecutionValidationResult`, `RuntimeTelemetry`), using capability-federation terminology. Add positive/negative fixtures proving a provider-specific name cannot become a required action dependency. Per the continuous-supervision amendment, each long external action (CI) is polled to terminal state within the turn.
