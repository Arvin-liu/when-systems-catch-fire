# 点火项目阅读导航

> 当前快照：2026-07-29，任务 101 人类阅读层与持续自我纠错引擎。历史版本和关闭证据仍在 Git、CHANGELOG 与原报告中保留。

## 先看结果

- [人类阅读总入口](./HUMAN-READING.md)
- [当前结果](./RESULTS/LATEST.md)
- [纠正与撤回](./RESULTS/CORRECTIONS.md)
- [开放问题](./RESULTS/OPEN-QUESTIONS.md)
- [函数与断言裁决总结](./RESULTS/ADJUDICATION-SUMMARY.md)
- [研究、复算、审计与文章](./RESULTS/RESEARCH-AND-ARTICLES.md)
- [完整历史结果台账](./RESULTS/CHRONOLOGY.md)

## 现行权威

- [README](./README.md)：可直接看到当前结论、纠正、开放问题和主要入口。
- [项目现状](./docs/project-current-state.md)：当前仓库能力、边界与未完成义务。
- [点火迭代操作法](./ITERATION.md)：状态变化、传播、验证、审查、合并与回执规则。
- [现行架构](./ARCHITECTURE.md) 与 [Foundation](./FOUNDATION.md)。
- [生命共同体价值宪章](./docs/governance/life-community-value-charter.md)：规范性边界。
- [断言治理与函数身份](./docs/foundation/claim-governance-and-function-identity.md)。
- [未来断言准入协议](./docs/foundation/future-claim-admission-protocol.md)。
- [自我纠错引擎](./docs/governance/self-correction-engine.md)。

## 架构与模型

点火以 L0 来源与证据、L1 受控命题、L2 形式对象、L3 逻辑论证、L4 数学模型与证明、L5 验证与有效性、L6 解释/应用/出版组织仓库。L6 只能引用 L0—L5，不能反向制造真值。

- [Function OS 候选](./function-os-candidate/v0.2/README.md)
- [MCF](./docs/architecture/multiscale-causal-fabric.md)
- [PSD](./docs/architecture/probabilistic-system-dynamics.md)
- [ARN](./docs/architecture/adaptive-relational-network.md)
- [系统图](./docs/architecture/interactive-system-map.md)

这些构件是表示、操作和审计能力；不是被外部证实的统一理论，也不证明现实因果、同构或完整性。

## 机器证据

- [函数资产 closure summary](./data/foundation/function-assets/closure-summary.json)
- [非函数断言 closure summary](./data/foundation/nonfunction-claims/closure-summary.json)
- [Claim Delta](./data/governance/self-correction/claim-delta.jsonl)
- [影响分析](./data/governance/self-correction/impact-analysis.jsonl)
- [证据谱系变化](./data/governance/self-correction/evidence-lineage-delta.jsonl)
- [自纠错审计发现](./data/governance/self-correction/audit-findings.jsonl)

机器记录与人类结果必须成对存在。机器记录可复算，但不自动拥有更高真值；人类摘要可阅读，但不覆盖机器来源或原报告。

## 历史与参与

- [版本历史](./CHANGELOG.md)
- [参与说明](./docs/participate.md)
- [贡献指南](./CONTRIBUTING.md)
- [许可作用域](./LICENSES/README.md)

独立部署阅读站已经退出维护。当前人类阅读层完全由仓库 Markdown 构成，重要内容不得默认折叠。
