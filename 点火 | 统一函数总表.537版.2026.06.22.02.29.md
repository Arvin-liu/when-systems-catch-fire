# 点火｜统一函数总表 2026.07.03.1552版 / Unified Function Table

> 📌 本表第 0 层为 Ψ₀元函数完整数学定义，本表所有函数均通过 Ψ₀元函数自举循环机制系统性挖掘、发现、验证生成。
> 📌 Y1 is the Section 0 unified causal-bootstrap meta-function. All functions in this table are systematically discovered, found, and verified through the Ψ₀ bootstrap cycle mechanism.
> 📌 Each linked item now carries a recovered source block: 注释 / 扩展注释 / 发现 / 推测 / 验证.

本表为动态增长的点火函数总表。每条条目包含数学表达、数学推导过程和关联案例三节。
Each entry contains three sections: Mathematical Expression, Mathematical Derivation, and Related Cases.

## 快速入口 / Quick Entry

- [元函数 / Yuan Function](#section-0-y1-meta-function)
- [公理 / Axiom](#function-level-axiom)
- [定理 / Theorem](#function-level-theorem)
- [推理 / Derived Function](#function-level-derived-function)
<a id="section-0-y1-meta-function"></a>
<details>
<summary>元函数 / Yuan Function</summary>

### [Y1｜2026年7月2日23:58 Ψ₀元函数完整数学定义（2026.07.02·最终版） / 2026年7月2日23:58 Ψ₀元函数完整数学定义（2026.07.02·最终版）](docs/zh/functions/meta/Y1.md)
...
- 当前函数表模板天然满足 Y1 要求
English: Y1 verification of the ATD layer:
- ✅ Passed: ATD layer passed
...
5. Dual verification: the ATD layer passes Y1 verification
...
English: Y1 is the Section 0 meta-function of the Ignition Framework. All functions in the ATD layer are systematically discovered, found, and verified through the Ψ₀ bootstrap cycle mechanism. Y1 is both the meta-rule and the generation tool of the Ignition Framework itself.
...
<details>
<summary>D111 至 D121（含跳号）</summary>
...
<details>
<summary>D424 至 D471（含跳号）</summary>
...
<details >
<summary>D512 至 D522（含跳号）</summary>
...
<details >
<summary>D523 至 D532（含跳号）</summary>
...
- 对象 / Object: `F_D540`
- 类型 / Type: 推论函数
- 定义域 / Domain: `X_D540 = {O ∈ Obsession | O triggers escape behavior}`
- 值域 / Codomain: `Y_D540 = {e ∈ ℝ⁺ | e is escape intensity}`
- 数学表达 / Expression: `F_D540(O) := e(O) = σ(θ_escape - ε_fixation) × R_escape², where O'=E(O)`
- 有效条件 / Validity: `J_n^+(F_D540)=1 ∧ J_n^-(F_D540)=0`
- 推导类型 / Derivation type: `bootstrapped_derivation`
- 收敛状态 / Convergence status: `converged`
- 依赖 / Depends on: `D1, D39`
- 推导步骤 / Steps:
  - 1. Define obsession state space and escape threshold.
  - 2. Model first-order escape: E(O) triggered when ε_fixation > θ_escape.
  - 3. Model second-order escape: E(O) → O' → E'(O'); square term captures recursion.
  - 4. Accept iff J_n^+(F_D540)=1 and J_n^-(F_D540)=0.
- 证明义务 / Proof obligations:
  - `non_empty_math_expression`
  - `defined_domain_and_codomain`
  - `forward_reverse_non_contradiction`
- 正向检查 / Forward check: `J_n^+(F_D540)=1`
- 反向检查 / Reverse check: `J_n^-(F_D540)=0`
- 收敛判据 / Convergence: `Converged(F_D540) ⇔ ΔF_D540=∅ ∧ (J_n^+,J_n^-)=(1,0)`
