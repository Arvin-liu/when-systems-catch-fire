...
- R5-A Life Integrity 仍是堆叠 Draft。Night Queue R1 保持 `NIGHT_QUEUE_R1_PARTIAL_SALVAGE_NOT_ACCEPTED`；已接受的窄修复 `a2fd355f…` / merge `062f223f…` 只关闭此前 30 个实例。完整 PR #130 精确头 `f33be64…` 的独立审查又复现 `R5A-CR-001`–`011` 十一个运行时/公开 schema 失败关闭旁路，故裁决为 `R5A_CONSOLIDATED_EXACT_HEAD_REJECTED_NARROW_REPAIR_REQUIRED`；读取 `docs/governance/r5a-consolidated-exact-head-review-r1.md`。PR #130 未 Ready、未合并到前驱、非 Main/Current；只允许一个窄修复请求，R5-B、R5-C、R6 禁止启动。
