...
- R5-A Life Integrity 仍是堆叠 Draft。已接受的 30-case 窄修复不等于完整候选通过；PR #130 精确头 `f33be64…` 的后续复核又发现 `R5A-CR-001`–`011` 十一个运行时/标准 Schema 旁路。唯一授权的合并合同旁路修复已在 Draft 中建立精确逐案机器门、六份 Draft 2020-12 正反实例门和五类变异阻断，但状态仍是 `R5A_CONSOLIDATED_CONTRACT_BYPASS_NARROW_REPAIR_DRAFT_AWAITING_REVIEW`。读取 `docs/governance/r5a-consolidated-contract-bypass-repair-r1.md`；不得用测试总数、绿色 CI 或确定性生成替代独立 exact-head 验收。PR #130 仍未 Ready/合并、非 Main/Current；R5-B、R5-C、R6 未启动且禁止启动。
