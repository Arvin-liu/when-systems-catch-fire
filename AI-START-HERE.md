...
若任务涉及 L6 公共故事或文章，还应读取 `docs/publication/cognitive-level-leap-writing-method.md` 与对应后台规格。121Q28R `0.2.0` 未经独立复审和合并前只是一项 Draft 候选，不是当前能力；`0.1.0` 已被反例拒绝，从未成为 current。
...
- 使用 121Q28R 候选时，必须落实起始承载点、“在下一层中写当前层”、心智引力、递归重定义或其他真实连续性、回照／反向显影和信息增益停止。肉身锚定是可选模式；无论入口为何，都保留现实成本与 Charter Gate 主体。
- 效果推理只产生行动候选，不产生真值。
