#!/usr/bin/env python3
"""repair-r3 adversarial closure test for latent_system_identifiability (RB09-CALLER-ASSERTED-SEMANTICS).

Builds a valid positive bundle from REAL Git objects, runs the wired latent_system_identifiability gate,
and asserts:
...
The gate's CONFIG["evaluator"] = evaluate_latent_system_identifiability recomputes every rule from record
...
GATE = REPO / "tools/latent/validate_latent_system_identifiability_gate.py"
CAP = "latent_system_identifiability"
...
class LatentSystemIdentifiabilityR3Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.config = _config()
