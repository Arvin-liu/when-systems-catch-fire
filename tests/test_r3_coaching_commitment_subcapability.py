#!/usr/bin/env python3
"""repair-r3 adversarial closure test for coaching_commitment_subcapability (RB09-CALLER-ASSERTED-SEMANTICS).

Builds a valid positive bundle from REAL Git objects, runs the wired coaching_commitment_subcapability gate,
and asserts:
...
The gate's CONFIG["evaluator"] = evaluate_coaching_commitment_subcapability recomputes every rule from record
...
GATE = REPO / "tools/coaching/validate_coaching_commitment_subcapability_gate.py"
CAP = "coaching_commitment_subcapability"
...
class CoachingCommitmentSubcapabilityR3Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.config = _config()
