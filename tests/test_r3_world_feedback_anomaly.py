#!/usr/bin/env python3
"""repair-r3 adversarial closure test for world_feedback_anomaly (RB09-CALLER-ASSERTED-SEMANTICS).

Builds a valid positive bundle from REAL Git objects, runs the wired world_feedback_anomaly gate,
and asserts:
...
The gate's CONFIG["evaluator"] = evaluate_world_feedback_anomaly recomputes every rule from record
...
GATE = REPO / "tools/anomaly/validate_world_feedback_anomaly_gate.py"
CAP = "world_feedback_anomaly"
...
class WorldFeedbackAnomalyR3Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.config = _config()
