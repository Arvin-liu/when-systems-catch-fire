...
class IterationSyncTests(unittest.TestCase):
    ...
    def test_current_draft_status_is_rejected(self):
        manifest = {
        ...
        with self.assertRaisesRegex(AssertionError, "current cannot be true"):
            validate_custom(manifest, __file__)

    def test_front_door_decision_is_required_for_operations_method(self):
        valid = {
            "method_version": "1.0.0",
            "task_id": "BAD2",
            "change_classification": ["OPERATIONS_METHOD"],
        ...
        with self.assertRaisesRegex(AssertionError, "CHANGELOG.md"):
