#!/usr/bin/env python3
"""repair-r3 adversarial closure test for decision_integrity (RB09-CALLER-ASSERTED-SEMANTICS).

Builds a valid positive bundle from REAL Git objects, runs the wired
decision_integrity gate, and asserts:
...
The gate's CONFIG["evaluator"] = evaluate_decision_integrity recomputes every rule
from record values + authoritative evidence bytes; caller facts/status are ignored.
"""
import ast
import json
import unittest
...
GATE = REPO / "tools" / "decision" / "validate_decision_integrity_gate.py"
CAP = "decision_integrity"
...
class DecisionIntegrityR3Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.config = _config()
    ...
    def test_semantically_false_but_git_valid_fails(self):
        b = self._bundle()
        # Flip the first rule's bound record value so it no longer corroborates
        # the evidence bytes (record value is NOT truth authority).
