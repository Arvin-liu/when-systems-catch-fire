#!/usr/bin/env python3
"""repair-r3 adversarial closure test for epistemic_state_control_plane (RB09-CALLER-ASSERTED-SEMANTICS).

Builds a valid positive bundle from REAL Git objects, runs the wired
epistemic_state_control_plane gate, and asserts:
...
The gate's CONFIG["evaluator"] = evaluate_epistemic_state_control_plane recomputes
every rule from record values + authoritative evidence bytes; caller facts/status
are ignored.
"""
import ast
import json
...
GATE = REPO / "tools" / "metacognition" / "validate_epistemic_state_control_plane_gate.py"
CAP = "epistemic_state_control_plane"
...
class EpistemicStateControlPlaneR3Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.config = _config()
