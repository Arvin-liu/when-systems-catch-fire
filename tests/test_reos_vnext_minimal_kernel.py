...
SUMMARY = {
    "exact_question": "Does the bounded pilot proposition hold under its stated scope?",
    "scope": "public non-high-stakes pilot",
    "estimands": ["bounded descriptive difference"],
    "measurement_distinctions": ["observed measure vs proxy"],
    "alternatives": ["selection", "measurement mismatch"],
    "claim_ceiling": "source-scoped descriptive result only",
    "source_family_target": 3,
...
def make_case():
    return new_case(
        ...
        question_ref="pilot/preregistration.json",
...
class MinimalKernelTests(unittest.TestCase):
    ...
    def test_new_case_is_valid_and_serialization_is_deterministic(self):
        ...
        self.assertEqual(document["case"]["question_contract"]["question_digest"], sha256_json(SUMMARY))
    ...
    def test_question_amendment_is_append_only(self):
        amended = amend_question(
            make_case(),
            summary={**SUMMARY, "scope": "narrower public pilot scope"},
        ...
        mutated["case"]["question_contract"]["summary"]["scope"] = "silent mutation"
