#!/usr/bin/env python3
"""repair-r3 adversarial closure test for graded_intervention_escalation (RB09-CALLER-ASSERTED-SEMANTICS).

Builds a valid positive bundle from REAL Git objects, runs the wired graded_intervention_escalation gate,
and asserts:
...
The gate's CONFIG["evaluator"] = evaluate_graded_intervention_escalation recomputes every rule from record
...
GATE = REPO / "tools/escalation/validate_graded_intervention_escalation_gate.py"
CAP = "graded_intervention_escalation"
...
class GradedInterventionEscalationR3Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.config = _config()
