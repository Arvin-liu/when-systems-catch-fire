...
class ApplyAuthorityPreflightProbe(unittest.TestCase):
    ...
    def test_preflight_rejects_nonautomatic_order_zero_side_effects(self): self.reject_zero(lambda p:(p['execution_order'].__setitem__(0,'pages_pipeline'),[d.update(decision='REBUILD') for d in p['component_decisions'] if d['component_id']=='pages_pipeline'],self.reseal(p)))
