#!/usr/bin/env python3
"""repair-r3 adversarial closure test for open_scientific_context_protocol (RB09-CALLER-ASSERTED-SEMANTICS).

Builds a valid positive bundle from REAL Git objects, runs the wired open_scientific_context_protocol gate,
and asserts:
...
The gate's CONFIG["evaluator"] = evaluate_open_scientific_context_protocol recomputes every rule from record
...
GATE = REPO / "tools/context_protocol/validate_open_scientific_context_protocol_gate.py"
CAP = "open_scientific_context_protocol"
...
class OpenScientificContextProtocolR3Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.config = _config()
