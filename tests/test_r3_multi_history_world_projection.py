#!/usr/bin/env python3
"""repair-r3 adversarial closure test for multi_history_world_projection (RB09-CALLER-ASSERTED-SEMANTICS).

Builds a valid positive bundle from REAL Git objects, runs the wired multi_history_world_projection gate,
and asserts:
...
The gate's CONFIG["evaluator"] = evaluate_multi_history_world_projection recomputes every rule from record
...
GATE = REPO / "tools/multihistory/validate_multi_history_world_projection_gate.py"
CAP = "multi_history_world_projection"
...
class MultiHistoryWorldProjectionR3Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.config = _config()
