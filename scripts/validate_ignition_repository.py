...
def check_readme(errors: list[str]) -> None:
    ...
    positioning_required = [
        "《点火》不是一本固定成书，也不是单个人类传记式项目，而是一个开放维护的函数、案例、发现、预测、新答案与解析解知识库。",
    ...
    required = [
        ...
        "[解析解 / Analytic Solutions](ANALYTIC_SOLUTIONS.md)",
...
def check_meta_functions(errors: list[str]) -> dict[str, int]:
    ...
    expected_ordinary = len(read_discovery_json(REPO_ROOT / "data/functions/unified-functions.json", []))
    if counts["functions"].get("ordinary") != expected_ordinary:
        errors.append(
            "repository overview ordinary function count mismatch: "
            f"{counts['functions'].get('ordinary')} != {expected_ordinary}"
        )
...
def check_presence(errors: list[str]) -> None:
    expected = [
        ...
        REPO_ROOT / "data/analytic-solutions/unified-analytic-solutions.json",
        REPO_ROOT / "data/analytic-solutions/unified-analytic-solutions.jsonl",
        REPO_ROOT / "data/analytic-solutions/unified-analytic-solutions-index.md",
        REPO_ROOT / "ANALYTIC_SOLUTIONS.md",
        REPO_ROOT / "docs/zh/analytic-solutions/items",
        REPO_ROOT / "docs/zh/analytic-solutions/categories",
        REPO_ROOT / "data/object-classification/mathematical-object-classification-rules.json",
        REPO_ROOT / "data/object-classification/object-classification-crosswalk.json",
        REPO_ROOT / "data/object-classification/object-reclassification-report.json",
        REPO_ROOT / "data/project-identity/project-positioning-lock.json",
        REPO_ROOT / "data/project-identity/project-positioning-lock.md",
        REPO_ROOT / "data/project-identity/project-evaluation-output-lock.json",
        REPO_ROOT / "data/project-identity/project-evaluation-output-lock.md",
        REPO_ROOT / "data/novelty-gate/universal-academic-novelty-report.json",
        REPO_ROOT / "data/novelty-gate/universal-academic-novelty-report.md",
        ANSWERS_LIST_MD,
        ANSWERS_INDEX_MD,
        ANSWER_TEMPLATE_MD,
