...
R5-A Life Integrity 的 Night Queue R1 报告未获接受：`NIGHT_QUEUE_R1_PARTIAL_SALVAGE_NOT_ACCEPTED`。交接时读取 `docs/governance/r5a-night-queue-r1-adjudication.md`、`docs/governance/r5a-consolidated-exact-head-review-r1.md` 与生成的攻击实例/验收回执。窄修复精确头 `a2fd355f…` / merge `062f223f…` 已关闭此前 30 个实例，但完整 PR #130 精确头 `f33be64…` 的独立复核又稳定复现 `R5A-CR-001`–`011` 十一个运行时/标准 JSON Schema 旁路，终态为 `R5A_CONSOLIDATED_EXACT_HEAD_REJECTED_NARROW_REPAIR_REQUIRED`。不得以 177 tests、绿色 CI 或确定性生成覆盖这些实例；PR #130 未 Ready/合并，当前只允许一个实例绑定的窄修复请求。R5-B、R5-C、R6 禁止启动。
## 许可边界
