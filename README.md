...
121Q28R 在同一隔离 Draft PR 中将[之元写作法](./docs/publication/cognitive-level-leap-writing-method.md)修订为 `0.2.0`：从可被后层重释的起始承载点出发，在下一层中写当前层，以不可容纳残余打开跃迁，通过递归重定义、双重归属铰链或其他隐形连续性进入新层，让潜题生长并反向显影，在无信息增益时停止。肉身锚定是可选模式，跨域同构是应用类型，高维压缩结尾是可选形式。它仍只是等待独立复审的 L6 候选，不是当前能力、新架构／真值层、普遍审美规范或文学质量证明器。
...
- [L6 出版方法候选](./docs/publication/cognitive-level-leap-writing-method.md) — 说明起始承载点、前视写作、心智引力、递归重定义、突然跃迁、回照／反向显影与信息增益停止；肉身锚定和压缩结尾均为可选，未合并前不得写成当前能力。
