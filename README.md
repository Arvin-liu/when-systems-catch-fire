...
## → [故事与案例](#故事与案例)
## → [为什么这很重要](#为什么这很重要)
## → [如何使用 AI 理解和运用点火框架](#如何使用-ai-理解和运用点火框架)
## → [反证机制与失败案例库](#反证机制与失败案例库)
## → [完整函数定义](#完整函数定义)
## → [附录：使用说明（完整版）](#附录使用说明完整版)
...
1. 打开你习惯的 AI 助手，把这个仓库链接发给它。
2. 说清楚你真正想问的问题，以及你正在观察的现象。
3. 让 AI 基于仓库里的函数、案例和反例材料，帮你生成解释、比较视角和可检验的结论。

你可以直接试这些提示：

- `我想了解美国的大萧条发生的原因，请你用点火框架分析。`
- `请基于点火项目判断互联网普及对民主制度的正负影响有哪些？`

如果你愿意手动操作，也可以按下面这条最小路径来走：
...
- 逐项做六通道检查
- 提出假设与反例
- 输出 `true / false / contradiction / pending`

更完整的手动流程仍然收在后面的折叠附录里，想深入时可以再展开。
...
## 故事与案例
真实故事最容易让人看见“点燃”是怎么发生的。下面这些卡片都来自仓库里的新故事或统一案例表，它们讲的不是抽象定义，而是人、选择和后果。

- **比刀剑更持久的，是共享观念**  
  拔都的权力看起来像征服，骨子里却靠共享和契约维系。真正管用的不是更重的刀，而是让人愿意留下来的规则。  
  [阅读全文](./新故事/0001-S1-比刀剑更持久的，是共享观念.md)
...
## 如何使用 AI 理解和运用点火框架
这个项目本来就是给好奇心驱动的探索用的，你不需要先读懂任何数学公式，也可以先让 AI 帮你看出结构和方向。

1. 打开你习惯的 AI 助手，把这个仓库链接发给它。你可以直接从 [仓库首页](.) 进入再复制链接。
2. 说清楚你真实想问的问题，或者你正在观察的现象，比如一条社会新闻、一个历史转折，或者一个技术趋势。
3. 告诉 AI：请基于这个项目里的函数和案例来分析，帮我找新视角、新解释和可能的反例。

AI 也可以不只帮你解释，还可以帮你设计反证测试、寻找反例、整理失败案例。你可以直接这样问：`请基于仓库帮我设计一个反例，检验这个假说是否成立`。

AI 会自己阅读函数索引和案例索引，自动提取结构并给出分析。你不需要先啃完数学部分，也不需要先搞懂所有术语；如果愿意，你也可以先看上面的故事与案例，或者去“如何开始”和折叠的使用说明书里做手动分析。

AI 的建议只是启发式，不是权威结论。凡是涉及医学、法律、金融、公共事件或现实人物的问题，AI 必须引用可靠外部资料；证据不足时，应明确标注为 `pending` 或“证据不足”。
...
### 4. 如果你想借助 AI
优先看上方的 AI 使用指南；这份折叠手册保留的是更完整的手动分析流程，方便你在需要时自己一步步推。
...
欢迎贡献！欢迎查看 [CONTRIBUTING.md](./CONTRIBUTING.md) 了解如何提交故事、函数、测试和反例。
