...
121Q28S 在同一隔离 Draft PR 中将[之元写作法](./docs/publication/zhiyuan-writing-method.md)修订为 `0.3.0`：它是点火同源认知结构在 L6 公共表达中的候选投影，也是把作品与读者反馈经 provenance 登记送回来源、gap、机制和现实反馈环的接口。心智层级跃迁仍是核心生成运动；肉身锚定、跨域同构和高维压缩分别是可选模式、应用和形式。同源仅表示维护者声明的共同设计来源与可审计结构对应候选，不等于脑科学事实、形式同构、新架构／真值层或文学质量证明。
...
- [L6 公共表达与反馈投影候选](./docs/publication/zhiyuan-writing-method.md) — 说明点火与之元写作法的有边界同源对应、向外表达与返回点火双向契约，以及心智层级跃迁内核；未合并前不得写成当前能力。
