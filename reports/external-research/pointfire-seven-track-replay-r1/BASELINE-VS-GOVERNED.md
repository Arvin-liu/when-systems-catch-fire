...
Owner decision: `ACCEPT_BOUNDED_REPLAY_RESULT_WITH_EXPLICIT_RESIDUALS`. The comparison is retained as a bounded research result; any future repair must update the source and claim ledgers without deleting the blinded baseline or rewriting a negative or boundary result. `EPISTEMICALLY_ACCEPTED=0`; external validity remains `NOT_ESTABLISHED`.
