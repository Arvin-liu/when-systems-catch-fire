...
class N6TraceCapture:
    ...
    def _compute_trace_hash(self, trace: dict) -> str:
        # Deterministic content fingerprint: excludes non-deterministic
        # trace_id (instance id) and timing_ms (wall-clock), so the same
        # execution content always yields the same hash regardless of order.
