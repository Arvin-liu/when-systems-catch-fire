...
Status: `CANDIDATE_NOT_PUBLISHED_TO_MAIN`

This page is a public-safe candidate on
`publication/pointfire-seven-track-results-update-r1-20260813`, based on
...
results below preserve scope, denominator, evidence type, uncertainty, and
residuals. None is `EPISTEMICALLY_ACCEPTED`.

### 1. External cross-domain replay

Frozen question:

> 在城市尺度，增加树冠覆盖是否能稳定降低夏季近地面热暴露？在什么条件下这一关系不能被解释为因果？

The blinded baseline was prepared before the governed pass. The sealed replay
has `7` substantive source families plus `2` contextual/method records, with
retrieval summary `5 / 3 / 1` (directly readable / abstract or authoritative
landing-page with limits / official PDF extraction limited). The final fresh
ROLE-C review returned `PASS`.

The bounded result is that measured canopy-cover exposures were associated with
lower local summertime near-surface air temperature in the Tacoma, Portland,
and Madison settings under their observational designs. This does not identify
a universal city-scale causal planting effect. LST, near-surface air
temperature, and human heat exposure were kept separate; direct human
heat-stress exposure was `SOURCE_NOT_RECOVERED`. Canopy volume, tree presence,
shade, and generic vegetation were not substituted for canopy cover.

Source attribution is retained in the replay ledger, including [Schwaab et al.](https://www.nature.com/articles/s41467-021-26768-w),
...
Compared with the blinded baseline, the governed pass added source-family
identity, retrieval status, source-boundary records, per-source causal audits,
contradiction traceability, abstention receipts, and a replay receipt. The
baseline already had endpoint separation, alternative explanations, a causal
ceiling, and a conditional conclusion. The measured architecture status is
...
Exactly `64` assets were frozen before adjudication: `32` function assets and
`32` nonfunction claims. Independent adjudication returned:

| Disposition | Function | Nonfunction | Total |
|---|---:|---:|---:|
| `SUPPORTED_BY_REPOSITORY_EVIDENCE` | 0 | 3 | 3 |
| `PARTIALLY_SUPPORTED` | 0 | 14 | 14 |
| `INTERNAL_ONLY` | 0 | 14 | 14 |
| `UNRESOLVED_IDENTITY` | 0 | 0 | 0 |
| `BLOCKED_WITH_EVIDENCE` | 1 | 1 | 2 |
| `RETAIN_QUARANTINE_PENDING` | 31 | 0 | 31 |

Scores selected review priority; they were not treated as semantic evidence.
No high-impact function asset was promoted by centrality. Historical labels
...
Twelve high-information synthesis units were frozen. Independent source-family
verification produced `2` supported-within-source-scope units, `9` partial or
boundary-supported units, and `1` bridge-not-identifiable unit. The candidate
source-family maps are not independent proof of the private notes; private
...
The live repository had `84` open Draft PRs in the disposition snapshot. Two
independent repository reviewers found no item safe to close under the strict
superseded/historical rule, so `0` PRs were closed and no branches were
deleted. This is not a rejection of the historical work; it is a conservative
preservation decision.

A fresh current-main validation found the human front-door defect (missing
explicit required headings and closure links) and repaired it on a separate
maintenance candidate. The standalone front-door validator now passes. The
remaining Foundation/nonfunction/knowledge generated-output drift, nonfunction
tracked-path accounting (`3588` listed vs `3610` tracked), a stale system-map
...
This update is a candidate for Owner review. It does not modify the canonical
Results Book or formal main. It proposes no merge order until the Owner reviews
