...
class CurrentTaskAdvancementTests(unittest.TestCase):
    ...
    def test_advancement_moves_canonical_source_to_task134(self) -> None:
        updated, changed = advancement.advance_document(self.source, self.contract)
        self.assertTrue(changed)
        self.assertEqual(updated["current_task"]["task_id"], "IGNITION-20260822-134")
        self.assertEqual(updated["task_identity"]["latest_architecture_changing_task"], "IGNITION-20260821-129")
        self.assertEqual(updated["task_identity"]["previous_canonical_current_task"], "IGNITION-20260822-133")
        self.assertEqual(updated["task_identity"]["previous_formal_task"], "IGNITION-20260822-133")
