...
def copy_human_front_door_shell(destination: Path) -> None:
    ...
    The production validator test below also has a full-repository fixture. The
    helper remains for the small app-only materialization fixture, where the
    shell is copied beside the app and an ``ignition`` alias preserves the
    repository-relative links emitted by the real root shell.
    """
    shell = ROOT.parent / ".github"
    if shell.is_dir():
        shutil.copytree(shell, destination / ".github", dirs_exist_ok=True)
        alias = destination.parent / "ignition"
...
class ProductionProfileProbe(unittest.TestCase):
    def test_production_capability_contract_and_all_local_validators_run(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                # Validators are declared relative to the application root,
    ...
    def test_production_profiles_materialize_only_declared_outputs(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            copy_human_front_door_shell(checkout)
