"""Deterministic Current live-attempt projection derived only from the ledger."""
...
LIVE_CURRENT_PROJECTION_SCHEMA = "live-current-projection-r1"
...
def _schema_validate(document: Mapping[str, Any]) -> None:
    try:
        ...
        return
    schema_path = Path(__file__).resolve().parents[1] / "schemas/operations/live-current-projection-r1.schema.json"
    try:
    ...
    if errors:
        ...
        raise LiveCurrentProjectionError(f"live Current projection schema violation at {path}: {error.message}")
...
def _attempt_summary(record: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "capture_completeness": events["capture_completeness"],
        "structured_result_present": structured["present"],
...
def _validate_summary(summary: Mapping[str, Any]) -> None:
    required = {
        "sequence", "task_id", "dispatch_id", "attempt_id", "executor_id", "adapter_id",
        "state", "return_code", "timed_out", "evidence_completeness", "capture_completeness",
        "structured_result_present", "validator_status", "reconciliation_status", "record_hash",
    }
...
def validate_projection(document: Mapping[str, Any], *, check_digest: bool = True) -> dict[str, Any]:
    """Validate the public projection and return a JSON-safe copy."""
    ...
    if value.get("schema_version") != LIVE_CURRENT_PROJECTION_SCHEMA:
    ...
    required = {
        "schema_version", "contract_id", "source_ledger", "counts", "per_executor",
        "latest_attempt_per_executor", "latest_validated_completion", "current_live_ceiling",
        "obligation", "next_eligible_action", "attempts", "claim_ceiling", "projection_digest",
    }
    ...
    counts = value["counts"]
    count_fields = {
        "total_attempts", "validated_completion_count", "unreconciled_count",
        "observation_incomplete_count", "complete_evidence_count", "incomplete_evidence_count",
    }
    ...
    if value["latest_validated_completion"] is not None:
        _validate_summary(value["latest_validated_completion"])
...
def build_live_current_projection(
    ...
    *,
    ...
    """Build a deterministic public projection from one validated ledger."""

    ledger = LiveAttemptLedger(ledger_path)
    records = ledger.records()
    audit = ledger.audit()
    summaries = [_attempt_summary(record) for record in records]
    ...
    unreconciled = [
        summary for summary, record in zip(summaries, records)
        if record["reconciliation_status"] in {"OPEN", "REQUIRES_RECONCILIATION"}
    ]
    ...
    if unreconciled:
        ...
        action = {
            "status": "BLOCKED_UNTIL_RECONCILIATION",
            "action": "RECONCILE_UNRECOVERED_ATTEMPTS",
            "blocker_summary": (
                f"{len(unreconciled)} attempt(s) remain unreconciled and the validated completion count is {len(validated)}; "
                "no retry or completion claim is eligible from this projection."
            ),
        }
    elif validated:
        ...
        action = {
            "status": "STOP_AFTER_FIRST_VALIDATED_COMPLETION",
            "action": "STOP_LIVE_INVOCATION",
            "blocker_summary": "A validated completion is present; no additional live invocation is eligible.",
        }
    else:
        ...
        action = {
            ...
            "action": "RUN_DYNAMIC_EXECUTOR_ADMISSION",
            "blocker_summary": "No validated completion is present and no historical attempt is available to close the obligation.",
        }

    projection = {
        "schema_version": LIVE_CURRENT_PROJECTION_SCHEMA,
        ...
        "counts": {
            ...
            "validated_completion_count": len(validated),
        ...
        "latest_attempt_per_executor": latest_per_executor,
        "latest_validated_completion": validated[-1] if validated else None,
        "current_live_ceiling": ceiling,
        "obligation": {
            "obligation_id": "LIVE_EXTERNAL_INVOCATION",
            "state": obligation_state,
            "reason": (
                f"Ledger-derived live state: {len(unreconciled)} unreconciled attempt(s), "
                f"{len(validated)} validated completion(s), {state_counts.get('OBSERVATION_INCOMPLETE', 0)} observation-incomplete attempt(s)."
            ),
            "unreconciled_attempt_ids": [summary["attempt_id"] for summary in unreconciled],
        },
        "next_eligible_action": action,
        "attempts": summaries,
        "claim_ceiling": "Deterministic repository-local live attempt projection only; no external truth, production readiness, Owner acceptance or epistemic upgrade is inferred.",
    }
    projection["projection_digest"] = sha256_json(_unsigned(projection))
    return validate_projection(projection)


__all__ = ["LIVE_CURRENT_PROJECTION_SCHEMA", "LiveCurrentProjectionError", "build_live_current_projection", "validate_projection"]
