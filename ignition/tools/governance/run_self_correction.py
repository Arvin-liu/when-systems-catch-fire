...
def git_bytes(ref: str, path: str) -> bytes | None:
    git_path = path if path.startswith(("ignition/", ".github/", ".gitignore", "AGENTS.md", "LICENSE")) else f"ignition/{path}"
    result = subprocess.run(["git", "show", f"{ref}:{git_path}"], cwd=REPO_ROOT, capture_output=True)
    return result.stdout if result.returncode == 0 else None


def tracked_and_untracked() -> set[str]:
    raw = subprocess.check_output(["git", "ls-files", "--cached", "--others", "--exclude-standard", "-z"], cwd=REPO_ROOT)
    return {item.decode("utf-8")[len("ignition/"):] if item.decode("utf-8").startswith("ignition/") else item.decode("utf-8") for item in raw.split(b"\0") if item}


def changed_paths(base: str) -> list[str]:
    raw = subprocess.check_output(["git", "diff", "--name-only", "-z", base, "--"], cwd=REPO_ROOT)
    changed = {item.decode("utf-8")[len("ignition/"):] if item.decode("utf-8").startswith("ignition/") else item.decode("utf-8") for item in raw.split(b"\0") if item}
    current = tracked_and_untracked()
    base_files = set(
        subprocess.check_output(["git", "ls-tree", "-r", "--name-only", "-z", base], cwd=REPO_ROOT)
        .decode("utf-8")
        .split("\0")
    )
    base_files = {path[len("ignition/"):] if path.startswith("ignition/") else path for path in base_files}
