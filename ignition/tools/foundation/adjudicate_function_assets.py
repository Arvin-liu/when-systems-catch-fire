...
# Production fixtures may copy the application into a temporary Git worktree
# root; use that root for historical commit/path discovery when present.
...
PUBLIC_FRONT_DOORS = {
    ".github/README.md", "SUMMARY.md", "FOUNDATION.md", "ARCHITECTURE.md", "ITERATION.md",
...
def public_claims() -> tuple[list[dict], list[dict]]:
    public_paths = [
        path for path in subprocess.check_output(["git", "ls-files", "--cached", "--others", "--exclude-standard"], cwd=GIT_ROOT, text=True).splitlines()
