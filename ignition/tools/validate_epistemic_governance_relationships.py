...
REQUIRED_OBLIGATIONS={f"K{i}_{x}" for i,x in [(1,"FORMALIZATION_NOT_CONFIRMATION"),(2,"M_E_ORTHOGONALITY"),(3,"NO_AXIS_AUTO_UPGRADE"),(4,"SOURCE_FAMILY_INDEPENDENCE"),(5,"PUBLIC_CEILING"),(6,"DEPENDENCY_REVIEW"),(7,"NO_WORKFLOW_TRUTH"),(8,"SUSPENSION_NON_EQUIVALENCE"),(9,"APPEND_ONLY_REVISION"),(10,"NO_SECOND_AUTHORITY"),(11,"FEEDBACK_RETURNS_TO_L0"),(12,"TYPED_RELATIONS"),(13,"ASSERTION_NON_ESCALATION")]}
...
REQUIRED_SURFACES={".github/README.md","docs/architecture/epistemic-governance-kernel-and-federated-planes.md","docs/architecture/getnote-1329-epistemic-governance-pressure-test.md","PUBLICATIONS/pointfire-results-book/09-正式仓库最新成果.md","data/architecture/interactive-system-map.json","KNOWLEDGE/README.md"}
...
ROUTES={
 ".github/README.md":("PUBLICATIONS/pointfire-results-book/RESULT-REGISTRY.jsonl","publication.results_book","LOCAL_AUTHORITY_REFERENCE",None,"publication.privacy"),
...
def validate(doc, repo_root, schema=None):
    ...
    for a in auths.values():
        ...
        for p in a["canonical_paths"]:
            target = repository_root / p if p.startswith(".github/") else root / p
            if p.startswith("/") or ".." in Path(p).parts or not target.exists(): errors.append(f"{a['id']}: invalid or missing canonical path: {p}")
    ...
    for p,r in routes.items():
        for q in (p,r["registry"]):
            target = repository_root / q if q.startswith(".github/") else root / q
            if q.startswith("/") or ".." in Path(q).parts or not target.exists(): errors.append(f"public route missing canonical path: {q}")
    ...
    if obligations["K13_ASSERTION_NON_ESCALATION"]["coverage"] != "MACHINE_ENFORCED": errors.append("assertion non-escalation must remain machine-bound")
