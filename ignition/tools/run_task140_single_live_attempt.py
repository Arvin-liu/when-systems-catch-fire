#!/usr/bin/env python3
"""Run and durably record Task140's one authorized live synthetic attempt.
...
try:
    from tools.run_task140_live_admission import (
        AUTH_SOURCE,
        CODEX,
        CONTROL_REPO,
        DOCUMENT_ROOT,
    ...
    from run_task140_live_admission import (  # type: ignore
        AUTH_SOURCE,
        CODEX,
        CONTROL_REPO,
        DOCUMENT_ROOT,
...
LEASE_ID = "lease-ignition-140-codex-live-01"
...
def _accounting_policy() -> AccountingPolicy:
    identifiers = {
        "principal": "principal-140",
        "namespace": "namespace-140",
        "workspace": "DISPOSABLE_SYNTHETIC_READ_ONLY",
        "episode": "episode-140-live",
        "pack": "pack-live-140",
    ...
    return AccountingPolicy(
        ...
        workspace_namespace={"DISPOSABLE_SYNTHETIC_READ_ONLY": "namespace-140"},
    )


def _steering(observed_at: str) -> LiveSteeringBinding:
    capsule = IntentCapsule(
        capsule_id="capsule-intent-140-live",
        intent_id="intent-140-live",
        goal_id="goal-140-live",
        ...
        blocker_refs=(),
        temporal_refs=(),
        report_contract_refs=("report-contract-140-live",),
        minimal_context_refs=("fixture-ref-140-live",),
        namespace_ref="namespace-140",
        created_at=observed_at,
    )
    scope = BudgetScope(
        principal_id="principal-140",
        namespace_id="namespace-140",
        workspace_id="DISPOSABLE_SYNTHETIC_READ_ONLY",
        episode_id="episode-140-live",
        pack_id="pack-live-140",
        executor_id="external.codex",
    )
    return LiveSteeringBinding(
        capsule=capsule,
        why_next_ref="why-next-140-live-attempt",
        priority=7,
        profile_ref="profile-live-140",
        project_ref=TASK_ID,
        budget_scope=scope,
    )
...
def _coordinator(
    ...
    return LiveDispatchCoordinator(
        ...
        admission=admission,
        queue=WorkQueue(state / "queue.json", max_depth=2, profile_limits={"profile-live-140": 2}),
...
def _ledger_record(
    *,
    ...
    return {
        ...
        "claim_ceiling": "Task140 canonical host observation and independent synthetic validation only; no external truth, production readiness, Owner acceptance or epistemic acceptance is inferred.",
...
def run_once() -> dict[str, Any]:
    ...
    if any(record["task_id"] == TASK_ID for record in existing):
        raise SingleLiveAttemptError("Task140 already has a ledger attempt; refusing any second live dispatch")
    ...
    census = validate_path(ROOT / "data/operations/iterations/140/local-executor-census-r1.json", expected_task_id=TASK_ID, expected_step="09")
    ...
    capture_parent = Path(tempfile.mkdtemp(prefix="pointfire-task140-capture-"))
    retain_capture = False
    try:
        with tempfile.TemporaryDirectory(prefix="ignition-140-live-episode-") as directory:
            root = Path(directory)
            ...
            try:
                ...
                live_process: LiveProcessResult | None = None
                ...
                try:
                    ...
                    live_process = result.observation.process
                ...
                return {
                    ...
                    "claim_ceiling": "One canonical Task140 host observation was durably appended; independent Current projection and validator interpretation remain Step12 evidence, with no external truth or completion upgrade inferred.",
                }
            finally:
...
def main() -> int:
    ...
    parser.add_argument("--write", action="store_true", help="write the sanitized machine receipt after the one attempt")
    ...
    try:
        ...
        return 1
    if args.write:
        output = ROOT / "data/operations/iterations/140/step11-live-attempt.json"
        output.write_text(json.dumps({
            "schema_version": "ignition-140-step11-live-attempt-r1",
            "task_id": TASK_ID,
            "step": "11",
            "status": result["status"],
            "bounded_attempt_policy": {
                "max_distinct_executor_families": 2,
                "max_attempts_per_executor_family": 1,
                "blind_retry": "FORBIDDEN",
                "stop_on_first_live_readonly_validated_completion": True,
            },
            "result": result,
            "claim_ceiling": "Task140 canonical repository-local live-process observation and durable attempt record only; independent validation and Current projection are separate gates, and no external truth or completion upgrade is inferred.",
        }, ensure_ascii=False, indent=2, sort_keys=True) + "\n", encoding="utf-8")
        print("LIVE_ATTEMPT_RECEIPT_WRITTEN path=" + str(output.relative_to(REPO_ROOT)))
