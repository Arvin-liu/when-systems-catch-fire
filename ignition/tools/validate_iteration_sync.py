...
# The production repository keeps the application under ``ignition/`` while
# isolated validator fixtures may copy that application into a temporary Git
# worktree root.  Keep path/link resolution rooted at REPO_ROOT, but use the
# actual Git root for history reads in both layouts.
...
def validate_registry(registry: dict, source: Path = REGISTRY_PATH) -> dict[str, dict]:
    ...
    for item in surfaces:
        ...
        if not is_external:
            target = REPO_ROOT / item["locator"] if item["locator"].startswith(".github/") else ROOT / item["locator"]
            require(target.exists(), f"{source}: repository surface does not exist: {item['locator']}")
    return by_id
...
def _git_path_candidates(path: str) -> list[str]:
    """Return current and pre-root-normalization Git path spellings."""
    normalized = path.removeprefix("./")
    candidates = [normalized]
    if normalized.startswith("ignition/"):
        candidates.append(normalized.removeprefix("ignition/"))
    else:
        candidates.append(f"ignition/{normalized}")
    return list(dict.fromkeys(candidates))


def _git_show_text(ref: str, path: str) -> str:
    """Read a path from either the current or historical repository layout."""
    last_error: subprocess.CalledProcessError | None = None
    for candidate in _git_path_candidates(path):
        ...
        last_error = subprocess.CalledProcessError(result.returncode, result.args, result.stdout, result.stderr)
...
def _registry_version_commits() -> dict[str, str]:
    ...
    candidates = _git_path_candidates(str(REGISTRY_PATH.relative_to(ROOT)))
    try:
        log = subprocess.run(
            ["git", "log", "--all", "--format=%H", "--", *candidates],
    ...
    for sha in log:
        if not sha:
            continue
        try:
            raw = _git_show_text(sha, str(REGISTRY_PATH.relative_to(ROOT)))
            doc = json.loads(raw)
...
def _validate_seal_f12(seal: dict, source: Path) -> None:
    ...
    try:
        ...
        if _base_head:
            ...
            if _era_ref:
                try:
                    _era_text = _git_show_text(
                        _era_ref, "data/operations/generated-output-authority.json"
                    )
                    _authority_doc = json.loads(_era_text)
                except (json.JSONDecodeError, ValueError, subprocess.CalledProcessError, OSError):
                    _authority_doc = None
    ...
    if "system_map" in seal:
        ...
        if _era_ref:
            try:
                _era_text = _git_show_text(
                    _era_ref, "data/architecture/interactive-system-map.json"
                )
    ...
    for candidate in candidates:
        try:
            history = subprocess.run(
                ["git", "log", "--all", "--format=%H", "-1", "--", candidate],
                cwd=GIT_ROOT,
                capture_output=True,
                text=True,
                check=True,
...
def _era_aware_recompute(request: dict, era_ref: str):
    ...
    def _show(ref: str, path: str):
        try:
            return json.loads(_git_show_text(ref, path))
        except (json.JSONDecodeError, ValueError, subprocess.CalledProcessError, OSError):
            return None
...
def validate_custom(manifest: dict, source: Path, seal: dict, registry: dict[str, dict] | None = None) -> None:
    ...
    if manifest["method_version"] == "1.0.0" and classifications & TRIGGERING_CLASSIFICATIONS:
        for surface in LEGACY_FRONT_DOORS:
            aliases = {surface}
            if surface == ".github/README.md":
                # Pre-root-normalization manifests correctly retain the old
                # root path as historical evidence; current manifests use the
                # GitHub-supported .github landing path.
                aliases.add("README.md")
            require(aliases.intersection(decisions), f"{source}: missing front-door/current-state impact decision for {surface}")
