...
def validate(text: str | None = None) -> list[str]:
    ...
    required_phrases = (
        ...
        "真实 registry 派生出 16 个 playbooks",
        "visualization.render_derived_system_view",
        "PROVIDER_UNAVAILABLE_IN_CURRENT_ENVIRONMENT",
        "不得自动安装、替代、升级或把 provider 生成物写回 canonical source",
        "只要求分析架构而没有图请求时，不得因 provider 可用而自动渲染",
