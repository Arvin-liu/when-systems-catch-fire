#!/usr/bin/env python3
"""Freeze the no-inference admission gate for Task140's single live attempt."""
...
FORMAL_SHA = "d57251ae65d9416b17aa3cbd12519357c616c5ef"
CONTROL_TIP = "86df72bea772b61bea5e3c3837e48a2c46f06c76"
...
def _envelope(
    ...
    expires_at: str,
    *,
    dispatch_id: str = "dispatch-140-live-01",
    attempt_id: str = "attempt-140-live-01",
    phase: str = "step10-admission-only",
) -> LiveDispatchEnvelope:
    synthetic_ref = "fixture://IGNITION-20260826-140"
    return LiveDispatchEnvelope(
        ...
        workspace_ref="DISPOSABLE_TASK140_SYNTHETIC_FIXTURE",
...
def run_gate() -> dict[str, Any]:
    census_path = ROOT / "data/operations/iterations/140/local-executor-census-r1.json"
    ...
    projection = json.loads((ROOT / "data/operations/iterations/140/live-current-projection-r2.json").read_text(encoding="utf-8"))
    ...
    with tempfile.TemporaryDirectory(prefix="ignition-140-admission-") as directory:
        root = Path(directory)
        ...
        try:
            ...
            filesystem_probe = adapter._new_runtime_scratch("admission-140-probe")
            try:
            ...
            return {
                "schema_version": "ignition-140-step10-live-admission-r1",
                "task_id": TASK_ID,
                "status": "PASS",
                "census": census,
                ...
                "claim_ceiling": "Task140 repository-local live admission, capability lease, synthetic fixture and independent validator freeze evidence only; no external process inference, live completion, external truth, production readiness, Owner acceptance or epistemic acceptance is inferred.",
            }
        finally:
...
def main() -> int:
    ...
    if args.write:
        report.update({
            ...
            "formal_candidate_sha": FORMAL_SHA,
            "bounded_live_attempt_policy": {
                "selection_source": "fresh Task140 local-executor-census-r1",
                ...
                "family_selection_is_brand_neutral": True,
            },
            "safety": {
                ...
                "formal_or_control_workspace_used_as_child_workspace": False,
            },
        })
        output = ROOT / "data/operations/iterations/140/step10-live-admission.json"
        output.write_text(json.dumps(report, ensure_ascii=False, indent=2, sort_keys=True) + "\n", encoding="utf-8")
        print("LIVE_ADMISSION_GATE_WRITTEN path=" + str(output.relative_to(REPO_ROOT)))
