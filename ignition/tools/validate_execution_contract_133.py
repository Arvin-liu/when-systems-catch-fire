#!/usr/bin/env python3
"""Validate the live Task133 execution contract."""
...
SCHEMA_PATH = ROOT / "schemas/operations/execution-contract-133-r1.schema.json"
...
def validate(document: dict[str, Any] | None = None) -> list[str]:
    ...
    if contract["identity_impact"] != "PRESENTATION_ONLY":
        errors.append("Task133 identity impact must remain PRESENTATION_ONLY")
...
def main() -> int:
    ...
    errors = validate()
    if errors:
        print("EXECUTION_CONTRACT_133_INVALID", file=sys.stderr)
        for error in errors:
            print(f"- {error}", file=sys.stderr)
        return 1
    print("EXECUTION_CONTRACT_133_OK task_id=IGNITION-20260822-133")
    return 0
