...
机器索引：[`step07-routing-index.json`](../data/research/task172-routing-r1/step07-routing-index.json)。当前索引包含 24138 条 canonical-ID 路由记录；返回候选后必须回读 authority registry 做 fingerprint exact validation。
...
- 当前记录数：16317；示例：`FUNCTION_ASSET:A0`, `FUNCTION_ASSET:A0004`, `FUNCTION_ASSET:A0036`。
- 不适用：不能把命中标签写成事实、因果、证据支持、证明或 canonical promotion。

### `EVIDENCE_CHECK`

- 含义/可做什么：retrieve an asset whose evidence boundary needs checking。
- 当前记录数：16477；示例：`FUNCTION_ASSET:A0`, `FUNCTION_ASSET:A0004`, `FUNCTION_ASSET:A0036`。
- 不适用：不能把命中标签写成事实、因果、证据支持、证明或 canonical promotion。
...
- 当前记录数：14756；示例：`FUNCTION_ASSET:A0`, `FUNCTION_ASSET:A0004`, `FUNCTION_ASSET:A0036`。
- 不适用：不能把命中标签写成事实、因果、证据支持、证明或 canonical promotion。
