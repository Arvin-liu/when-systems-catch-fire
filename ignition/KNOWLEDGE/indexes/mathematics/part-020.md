...
- [证明某一 primitive 只是另一个局部字段的同义别名，删除它不会丢失任何独立 failure mode；](../../../docs/governance/more-meta-foundations-synthesis-2026-09-04.md)
  - 类型/状态：`NONFUNCTION_CLAIM` · `ACCEPTED_AS_DEFINITION`
  - 可搜索名称：NFC-d793032e930db50e / 证明某一 primitive 只是另一个局部字段的同义别名，删除它不会丢失任何独立 failure mode；
...
- [这不是一个今天新发明的“元元协议”。它是对仓库中已经反复运行的接口第一次做整体识别和显式绑定。独立反统一审查裁定：当前证据支持阶段性成熟的 federation，不足以证明一个共享状态机或单一生命周期意义上的统一架构。](../../../docs/architecture/epistemic-governance-kernel-and-federated-planes.md)
  ...
  - 依赖：—；被引用：—
- [这不是新增架构层，也不是“两个基础已经被证明”。它是一个研究结论：](../../../docs/governance/more-meta-foundations-synthesis-2026-09-04.md)
  - 类型/状态：`NONFUNCTION_CLAIM` · `ACCEPTED_AS_DEFINITION`
  - 可搜索名称：NFC-4c791ea9211ed37b / 这不是新增架构层，也不是“两个基础已经被证明”。它是一个研究结论：
  - 来源：`docs/governance/more-meta-foundations-synthesis-2026-09-04.md`
  - 依赖：—；被引用：—
...
- [这句话之所以扎人，不是因为它冒犯，而是因为它太准确。一个人当然可以同时是母亲、妻子、创业者、内容创作者，也可以心甘情愿地把很多时间给家庭、公司和别人。问题不在于她有没有这些角色。问题在于，当外部世界只通过这些角色来认识她时，她的名字会慢慢退](../../../outputs/stories/20260712-disobedience-subjectivity/story-longform.md)
  ...
  - 依赖：—；被引用：—
- [这张图只表示研究中的解释关系，不表示现实因果、数学推导或 registry dependency。`P` 约束“能说什么”，`N` 约束“不能以谁为代价”，`A` 约束“谁能决定并承载”，`O` 约束“在不确定时保留什么选择”。没有一项单独](../../../docs/governance/more-meta-foundations-synthesis-2026-09-04.md)
  - 类型/状态：`NONFUNCTION_CLAIM` · `ACCEPTED_AS_DEFINITION`
  - 可搜索名称：NFC-71f3ebdd97fb1eb2 / 这张图只表示研究中的解释关系，不表示现实因果、数学推导或 registry dependency。`P` 约束“能说什么”，`N` 约束“不能以谁为代价”，`A` 约束“谁能决定并承载”，`O` 约束“在不确定时保留什么选择”。没有一项单独
  - 来源：`docs/governance/more-meta-foundations-synthesis-2026-09-04.md`
  - 依赖：—；被引用：—
- [这是 `PROPOSAL`，不是新生命周期，也不是 L7。低风险、个人范围、可逆的动作可以在实践上合并若干步；高后果、公共、不可逆或影响沉默主体的动作不能用“流程很短”取消这些边界。](../../../docs/governance/accountable-cognition-boundary-map-2026-09-04.md)
  - 类型/状态：`NONFUNCTION_CLAIM` · `ACCEPTED_AS_DEFINITION`
  - 可搜索名称：NFC-102b464837a206f5 / 这是 `PROPOSAL`，不是新生命周期，也不是 L7。低风险、个人范围、可逆的动作可以在实践上合并若干步；高后果、公共、不可逆或影响沉默主体的动作不能用“流程很短”取消这些边界。
...
- [这说明肉身锚定可以强力参与跃迁，不证明每篇文章都必须以身体为入口或回返处。它与价值宪章共享宇宙尺度、主体位置、物理隔绝和伦理共在的来源，但诗的生成运动不等于 Charter 的规范权威。](../../../docs/publication/zhiyuan-writing-examples.md)
  ...
  - 来源：`docs/publication/zhiyuan-writing-examples.md`
  - 依赖：—；被引用：—
- [这里的 `CROSS_CONTRACT_LENS` 不是 `NEW_FAILURE` 的替代性断言：它表示当前材料足以说明“应将几张既有表面一起复核”，但没有独立复现证明该名字对应一个新的、稳定的错误类。](../../../docs/governance/more-meta-foundations-adversarial-audit-2026-09-04.md)
  - 类型/状态：`NONFUNCTION_CLAIM` · `ACCEPTED_AS_DEFINITION`
  - 可搜索名称：NFC-536dcd41ad3ebb1a / 这里的 `CROSS_CONTRACT_LENS` 不是 `NEW_FAILURE` 的替代性断言：它表示当前材料足以说明“应将几张既有表面一起复核”，但没有独立复现证明该名字对应一个新的、稳定的错误类。
