...
- The projection-hygiene gate fails on a clean `e04752d20d071bac8f0c4a1e5cff20fb3004dae1` baseline with `missing=164`; the Task131 receipt had recorded the earlier historical baseline as `missing=96`. At the Step11 measurement boundary the candidate reported `missing=192`. The delta was 28 newly added Task132 formal paths; Step12 records the two additional audit artifacts and the resulting `missing=194` separately.
...
The Task132 new residual is limited to projection-manifest accounting for its 28 newly added formal paths at this measurement boundary. No residual-owned Human Surface entries, durability pilot code, or architecture/map source was modified in this task.
