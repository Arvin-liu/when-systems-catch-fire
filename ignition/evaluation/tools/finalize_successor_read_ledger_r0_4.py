#!/usr/bin/env python3
"""Close an R0.4 successor ledger and attest to its frozen bytes."""
...
PACKET_DIR = IGNITION_ROOT / "evaluation" / "heldout" / "r0.4" / "successor-visible"
MANIFEST_PATH = PACKET_DIR / "packet-manifest-r0.4.json"
MANIFEST_REL = MANIFEST_PATH.relative_to(REPO_ROOT).as_posix()
LEDGER_SCHEMA_PATH = PACKET_DIR / "successor-read-ledger-r0.4.schema.json"
RECEIPT_SCHEMA_PATH = PACKET_DIR / "successor-ledger-validation-receipt-r0.4.schema.json"
SOURCE_PATH = Path(__file__).resolve()

TASK_ID = "IGNITION-20260919-188-R0.4"
LEDGER_SCHEMA_VERSION = "successor-read-ledger-r0.4"
MANIFEST_SCHEMA_VERSION = "heldout-successor-packet-manifest-r0.4"
RECEIPT_SCHEMA_VERSION = "successor-ledger-validation-receipt-r0.4"
PROTOCOL_VERSION = "r0.4"
VALIDATOR_COMPONENT_ID = "final-read-ledger-validator-r0.4"
...
def _entry_status(
    ...
    *,
    ...
    if action == "COGNITIVE_READ":
        ...
        # An authorized cognitive read is the intended way to expose a held-out
        # case to the successor; the case-specific bit does not override the
        # manifest path and digest authorization.
...
def finalize_files(ledger_path: Path, manifest_path: Path, receipt_path: Path) -> str:
    if (
        ...
        or receipt_path.name != "successor-ledger-validation-receipt-r0.4.json"
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--ledger", required=True, help="open or already closed R0.4 ledger")
    parser.add_argument("--manifest", default=str(MANIFEST_PATH), help="exact R0.4 packet manifest")
