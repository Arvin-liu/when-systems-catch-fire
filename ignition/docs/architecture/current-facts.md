...
- Foundation: function identity cards `5979`；function quarantine/pending `4869`；non-function claims `17304`；non-function quarantine/pending `4908`。
- Knowledge Experience: cards `414`；changes `315`；layered readings `332`；search records `23615`；aliases `876`。
...
- Task lineage: current `IGNITION-20260827-142` `IN_PROGRESS`；125 file `HISTORICAL_UNEXECUTED`, requirements `REBASED_INTO_127`, canonical `HISTORICAL_UNEXECUTED_REBASED_INTO_127`；127 `COMPLETED_WITH_CLASSIFIED_RESIDUALS`。
- Formal task lifecycle: `IGNITION-20260827-142` `IN_PROGRESS` terminal `false` scope_complete `false`；source `ignition/data/operations/formal-task-lifecycle-r1.json`。
- Formal terminal history: `[{'task_id': 'IGNITION-20260826-141', 'execution_status': 'COMPLETED_WITH_OPEN_OBLIGATIONS', 'terminal': True}]`；terminal formal tasks remain independent from open obligations。
