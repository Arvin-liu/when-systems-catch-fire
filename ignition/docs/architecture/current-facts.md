...
- Task lineage: current `IGNITION-20260828-147` `IN_PROGRESS`；125 file `HISTORICAL_UNEXECUTED`, requirements `REBASED_INTO_127`, canonical `HISTORICAL_UNEXECUTED_REBASED_INTO_127`；127 `COMPLETED_WITH_CLASSIFIED_RESIDUALS`。
- Formal task lifecycle: `IGNITION-20260828-147` `IN_PROGRESS` terminal `false` scope_complete `false`；source `ignition/data/operations/formal-task-lifecycle-r1.json`。
