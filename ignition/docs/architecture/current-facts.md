...
- Identity epoch: `os-control-plane-r3-durability-lifecycle-r0`；current iteration boundary: `128`。
- Architecture registry: `93` components；`81` visible map nodes；`12` hidden represented components；`125` typed relations；`85` visible typed edges。
- Map/method: map `0.11.0` Current（historical `0.10.0`）；layout `1.9.0`；semantic trunk `semantic-trunk-r1` with `6` bounded route stages；method `1.4.0` `Current`。
...
- Foundation: function identity cards `5611`；function quarantine/pending `4812`；non-function claims `16240`；non-function quarantine/pending `4706`。
...
- Task lineage: current `IGNITION-20260821-128` `COMPLETED_WITH_CLASSIFIED_RESIDUALS`；125 file `HISTORICAL_UNEXECUTED`, requirements `REBASED_INTO_127`, canonical `HISTORICAL_UNEXECUTED_REBASED_INTO_127`；127 `COMPLETED_WITH_CLASSIFIED_RESIDUALS`。
