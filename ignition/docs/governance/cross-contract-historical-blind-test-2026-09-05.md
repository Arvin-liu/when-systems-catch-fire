...
The first final-head Foundation CI run (`33940447499`) exposed a repository input-boundary issue: the existing official function-census and nonfunction-claim generators treated the newly tracked research corpus and its narrative/result surfaces as canonical discovery inputs. That made the generated census and claim outputs drift (the unbounded trial produced six extra function candidates and 176 extra nonfunction claim rows) even though Task155 did not intend to add production claims. The repair added task-scoped exclusions to those two official generators, retaining the paths in source-discovery accounting while preventing research labels from entering the authoritative function census or claim registry. No validator, gate, schema, runtime, authority or lifecycle rule changed. After regeneration, the official local Foundation validator returned `ALL_FOUNDATION_VALID` with `CHECKS_TOTAL=63 CHECKS_PASSED=63 CHECKS_FAILED=0`, and the generator `--check` commands were deterministic.

The next final-head Foundation run (`33942616041`) passed the core Foundation, function and nonfunction checks but stopped in the downstream human-result/self-correction bundle because its official projections had not yet been regenerated for the changed governance inputs. The repair used the repository-owned generators: task-scoped `build_human_results` exclusions keep the research casebook/report/result out of the human result ledger, while `run_self_correction`, `build_knowledge_experience`, `build_fire_seed_census` and `build_claim_browsers` refreshed their deterministic projections. The follow-up local checks pass, including `KNOWLEDGE_EXPERIENCE_AUDIT_OK` and `KNOWLEDGE_EXPERIENCE_DETERMINISM_OK`; this is generated input/projection maintenance and does not change a validator, gate, schema, runtime, authority or lifecycle rule. The failed run remains recorded as a residual until a new final-head CI run completes.
