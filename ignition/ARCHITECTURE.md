...
- current_formal_task: `IGNITION-20260828-147` (ordinal `147`)；status: `IN_PROGRESS`；terminal: `false`；latest_architecture_changing_task: `IGNITION-20260827-142` (ordinal `142`)；current_iteration_boundary: `147` is a deprecated compatibility alias of `current_formal_task_ordinal`；publication_witness_task: `IGNITION-20260828-147`。
- formal_task_terminality: authority `FORMAL_TASK_LIFECYCLE`；task `IGNITION-20260828-147` status `IN_PROGRESS`；terminal `false`；scope_complete `false`；open references `['LIVE_EXTERNAL_INVOCATION']`。
- formal_task_terminal_history: `[{"execution_status": "COMPLETED_WITH_OPEN_OBLIGATIONS", "task_id": "IGNITION-20260828-146", "terminal": true}, {"execution_status": "COMPLETED_WITH_OPEN_OBLIGATIONS", "task_id": "IGNITION-20260828-145", "terminal": true}, {"execution_status": "COMPLETED_WITH_OPEN_OBLIGATIONS", "task_id": "IGNITION-20260828-144", "terminal": true}, {"execution_status": "COMPLETED_WITH_OPEN_OBLIGATIONS", "task_id": "IGNITION-20260827-143", "terminal": true}, {"execution_status": "COMPLETED_WITH_OPEN_OBLIGATIONS", "task_id": "IGNITION-20260827-142", "terminal": true}, {"execution_status": "COMPLETED_WITH_OPEN_OBLIGATIONS", "task_id": "IGNITION-20260826-141", "terminal": true}]`；Task141 terminality remains recorded independently of the carried obligation。
...
- release_lifecycle: task `IGNITION-20260828-147`；content phase `RUNNING`；publication authority `REMOTE_REF_OBSERVATION`；embedded publication assertion `NONE`；required ref `refs/heads/main`；post-publication verification must observe that remote ref。
...
- task_lineage: current `IGNITION-20260828-147` `IN_PROGRESS`；predecessor `HISTORICAL_UNEXECUTED_REBASED_INTO_127` / `REBASED_INTO_127`；successor `COMPLETED_WITH_CLASSIFIED_RESIDUALS`。
- source: ignition/data/operations/current-snapshot-r1.json；source_digest: `f9b40d70a812cdd0a5ba9128efd5ede00337e5c5f0e0acdbaabb72024b3f56ad`。
...
completion authority. Historical live attempts are now read through the durable
append-only ledger: Hermes136 is TIMED_OUT_EFFECT_UNKNOWN, Codex137 is
FAILED_VALIDATION, Codex138 first is STARTUP_FAILURE, and Codex138 second is
OBSERVATION_INCOMPLETE. The ledger-derived live projection keeps
...
Task141 extends that capability into the R7 Live State Semantics / Structured
Result Reliability boundary. Dispatch observation, process observation,
independent inference markers, structured-result return and independent
validation are separate facts; `live_dispatch_calls=1` cannot promote
`inference_observation_status` to `OBSERVED`. The R3 Current projection now
recognizes Task140's real process while retaining zero validated completions,
and the FailureForensicsCapsule is written before raw private spool disposal.
Task140's malformed-result root cause is not confirmed by its retained formal
evidence, so no same-family Codex blind retry is admitted. This is an OS-owned
repository boundary, not provider runtime, external truth, production
readiness or epistemic acceptance.
