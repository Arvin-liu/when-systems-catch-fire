...
- [require(set(attestations) == external_ids, f"{source}: external attestation coverage mismatch")](../../../tools/validate_iteration_sync.py)
  ...
  - 来源：`tools/validate_iteration_sync.py`
  - 依赖：—；被引用：—
- [require(set(declared) == changed, f"{prefix}: silent or spurious frame changes observed={sorted(changed)} declared={sorted(set(declared))}", errors)](../../../tools/language_thought/validate_language_thought.py)
  - 类型/状态：`FUNCTION_ASSET` · `QUARANTINE_UNTIL_DEFINED`
  - 可搜索名称：IMPLICIT-756E97422DAAE753 / require(set(declared) == changed, f"{prefix}: silent or spurious frame changes observed={sorted(changed)} declared={sorted(set(declared))}", errors) / IMPLICIT-756E97422DAAE753
  - 来源：`tools/language_thought/validate_language_thought.py`
  - 依赖：—；被引用：—
- [require(set(ids) == dimension_ids, f"{prefix}: dimension basis mismatch", errors)](../../../tools/language_thought/validate_language_thought.py)
  - 类型/状态：`FUNCTION_ASSET` · `QUARANTINE_UNTIL_DEFINED`
  - 可搜索名称：IMPLICIT-871D915FECFE648A / require(set(ids) == dimension_ids, f"{prefix}: dimension basis mismatch", errors) / IMPLICIT-871D915FECFE648A
  - 来源：`tools/language_thought/validate_language_thought.py`
  - 依赖：—；被引用：—
- [require(set(ids) == set(REQUIRED_PROFILES), "manifest: required zh-Hans, en, ja and tr profiles missing or duplicated", errors)](../../../tools/language_thought/validate_language_thought.py)
  - 类型/状态：`FUNCTION_ASSET` · `QUARANTINE_UNTIL_DEFINED`
  - 可搜索名称：IMPLICIT-B5695D67A29DDBCC / require(set(ids) == set(REQUIRED_PROFILES), "manifest: required zh-Hans, en, ja and tr profiles missing or duplicated", errors) / IMPLICIT-B5695D67A29DDBCC
  - 来源：`tools/language_thought/validate_language_thought.py`
  - 依赖：—；被引用：—
...
- [require(set(ordered_ids) == visible, f"layout visibility mismatch: missing={sorted(visible-set(ordered_ids))} extra={sorted(set(ordered_ids)-visible)}")](../../../tools/generate_interactive_system_map.py)
  ...
  - 来源：`tools/generate_interactive_system_map.py`
  - 依赖：—；被引用：—
- [require(set(source_frame) == set(target_frame), f"{prefix}: source and target frames must annotate the same dimensions", errors)](../../../tools/language_thought/validate_language_thought.py)
  - 类型/状态：`FUNCTION_ASSET` · `QUARANTINE_UNTIL_DEFINED`
  - 可搜索名称：IMPLICIT-B4B7F6001984E209 / require(set(source_frame) == set(target_frame), f"{prefix}: source and target frames must annotate the same dimensions", errors) / IMPLICIT-B4B7F6001984E209
  - 来源：`tools/language_thought/validate_language_thought.py`
  - 依赖：—；被引用：—
...
- [return candidates\[0\] if len(candidates) == 1 else None](../../../evidence-program/tools/validate_evidence_program.py)
  ...
  - 依赖：—；被引用：—
- [return committed is not None and buf.getvalue() == committed](../../../tools/propagation/current_truth_projection.py)
  - 类型/状态：`FUNCTION_ASSET` · `QUARANTINE_UNTIL_DEFINED`
  - 可搜索名称：IMPLICIT-DB97D6D0434B4A7C / return committed is not None and buf.getvalue() == committed / IMPLICIT-DB97D6D0434B4A7C
...
- [same = canon(used) == canon(pre)](../../../evidence-program/tools/validate_evidence_program.py)
  ...
  - 来源：`evidence-program/tools/validate_evidence_program.py`
  - 依赖：—；被引用：—
- [same projection-check contract (D3 fix: shared CI gate).](../../../tests/test_current_truth_projection_parity.py)
  - 类型/状态：`NONFUNCTION_CLAIM` · `RETAINED_AS_STRUCTURAL_METAPHOR`
  - 可搜索名称：NFC-e7ba64efaff6a6e5 / same projection-check contract (D3 fix: shared CI gate).
  - 来源：`tests/test_current_truth_projection_parity.py`
  - 依赖：D3；被引用：—
- [sat = sum(1 for o in obls if o.get("status") == "SATISFIED")](../../../tests/test_deep_research_round5.py)
  - 类型/状态：`FUNCTION_ASSET` · `QUARANTINE_UNTIL_DEFINED`
  - 可搜索名称：IMPLICIT-867CBC75316390FA / sat = sum(1 for o in obls if o.get("status") == "SATISFIED") / IMPLICIT-867CBC75316390FA
...
- [self.assertFalse(any(p.get("validator_argv") == \["python3", "tools/validate_protocol_canonical.py", "--check"\] for p in profiles))](../../../tests/test_production_execution_authority.py)
  ...
  - 依赖：—；被引用：—
- [self.assertIn(tn, present, f"iteration {tn} must be folded into the projection")](../../../tests/test_current_truth_projection_parity.py)
  - 类型/状态：`NONFUNCTION_CLAIM` · `RETAINED_AS_STRUCTURAL_METAPHOR`
  - 可搜索名称：NFC-8b91898f8d3a4a49 / self.assertIn(tn, present, f"iteration {tn} must be folded into the projection")
  - 来源：`tests/test_current_truth_projection_parity.py`
  - 依赖：—；被引用：—
...
- [Space_Function(Ω) = {I(Ω), D_immune(Ω), R(Ω)}](../../../%E7%BB%9F%E4%B8%80%E5%87%BD%E6%95%B0%E6%80%BB%E8%A1%A8/0503-D505-%E7%A9%BA%E9%97%B4%E5%87%BD%E6%95%B0%E4%B8%89%E5%85%83%E6%8A%95%E5%BD%B1.md)
  ...
  - 依赖：—；被引用：—
- [stale_path = os.path.join(d, "stale-projection.json")](../../../tests/test_current_truth_projection_parity.py)
  - 类型/状态：`NONFUNCTION_CLAIM` · `RETAINED_AS_STRUCTURAL_METAPHOR`
  - 可搜索名称：NFC-4d472c6a802ae8d1 / stale_path = os.path.join(d, "stale-projection.json")
  - 来源：`tests/test_current_truth_projection_parity.py`
  - 依赖：—；被引用：—
...
- [The projection is derived ONLY from terminal (remotely verified) ledger records](../../../tools/propagation/current_truth_projection.py)
  ...
  - 来源：`tools/propagation/current_truth_projection.py`
  - 依赖：—；被引用：—
- [The projection only folds TERMINAL_SUCCESS tasks; a non-terminal / absent](../../../tests/test_current_truth_projection_parity.py)
  - 类型/状态：`NONFUNCTION_CLAIM` · `ACCEPTED_AS_DEFINITION`
  - 可搜索名称：NFC-a6d88cfd5a177d54 / The projection only folds TERMINAL_SUCCESS tasks; a non-terminal / absent
  - 来源：`tests/test_current_truth_projection_parity.py`
  - 依赖：—；被引用：—
...
- [tools/operations/run_stage_snapshot_responsibility_cases.py](../../../tools/operations/run_stage_snapshot_responsibility_cases.py)
  - 类型/状态：`FUNCTION_ASSET` · `QUARANTINE_UNTIL_DEFINED`
  - 可搜索名称：A16 / tools/operations/run_stage_snapshot_responsibility_cases.py / A16 / A17
  - 来源：`tools/operations/run_stage_snapshot_responsibility_cases.py`
  - 依赖：—；被引用：—
- [tools/operations/run_stage_snapshot_responsibility_cases.py](../../../tools/operations/run_stage_snapshot_responsibility_cases.py)
  - 类型/状态：`FUNCTION_ASSET` · `QUARANTINE_UNTIL_DEFINED`
  - 可搜索名称：A17 / tools/operations/run_stage_snapshot_responsibility_cases.py / A17 / A16
  - 来源：`tools/operations/run_stage_snapshot_responsibility_cases.py`
  - 依赖：—；被引用：—
- [tools/operations/run_stage_snapshot_responsibility_cases.py](../../../tools/operations/run_stage_snapshot_responsibility_cases.py)
  - 类型/状态：`FUNCTION_ASSET` · `QUARANTINE_UNTIL_DEFINED`
  - 可搜索名称：A18 / tools/operations/run_stage_snapshot_responsibility_cases.py / A18 / A16
  - 来源：`tools/operations/run_stage_snapshot_responsibility_cases.py`
  - 依赖：—；被引用：—
...
- [Validated repository-declared dependency planning, execution, validation and selective materialization of copyright risk](../../../reports/operations/121Q33-completion-seal.json)
  ...
  - 依赖：—；被引用：—
- [Versioned map projection and navigation; not a permanent single total map.](../../../docs/research-os/EXISTING-CAPABILITY-MAP.md)
  - 类型/状态：`NONFUNCTION_CLAIM` · `ACCEPTED_AS_DEFINITION`
  - 可搜索名称：NFC-0280953bcb14a6ae / Versioned map projection and navigation; not a permanent single total map.
...
- [中文：拓扑选择 = argmax{P_collapse, P_sustain} P_collapse = 1 - Πᵢ(1-fᵢ_risk)（风险规避模式） P_sustain = Πⱼfⱼ_cap（能力扩展模式） 选择判据： · 当min(fⱼ_cap) < θ_floor → P_sustain→0 → 选择F_collapse（二选一）](../../../%E7%BB%9F%E4%B8%80%E5%87%BD%E6%95%B0%E6%80%BB%E8%A1%A8/0114-D65-%E4%B9%98%E6%B3%95%E6%8B%93%E6%89%91%E9%80%89%E6%8B%A9%E5%87%BD%E6%95%B0.md)
  ...
  - 依赖：—；被引用：—
- [中文：收益-风险投影-网结构](../../../%E7%BB%9F%E4%B8%80%E6%A1%88%E4%BE%8B%E6%80%BB%E8%A1%A8/0079-C-79-%E6%94%B6%E7%9B%8A-%E9%A3%8E%E9%99%A9%E6%8A%95%E5%BD%B1-%E7%BD%91%E7%BB%93%E6%9E%84.md)
  - 类型/状态：`NONFUNCTION_CLAIM` · `HISTORICAL_ONLY`
  - 可搜索名称：NFC-08584eb73f0c1957 / 中文：收益-风险投影-网结构
  - 来源：`统一案例总表/0079-C-79-收益-风险投影-网结构.md`
  - 依赖：—；被引用：—
- [中文：收益-风险投影-网结构 English: Rule-based English rendering pending human review.](../../../%E7%BB%9F%E4%B8%80%E6%A1%88%E4%BE%8B%E6%80%BB%E8%A1%A8/0079-C-79-%E6%94%B6%E7%9B%8A-%E9%A3%8E%E9%99%A9%E6%8A%95%E5%BD%B1-%E7%BD%91%E7%BB%93%E6%9E%84.md)
  - 类型/状态：`NONFUNCTION_CLAIM` · `HISTORICAL_ONLY`
  - 可搜索名称：NFC-cf6ce1cde9afb87c / 中文：收益-风险投影-网结构 English: Rule-based English rendering pending human review.
  - 来源：`统一案例总表/0079-C-79-收益-风险投影-网结构.md`
  - 依赖：—；被引用：—
- [中文：智能 = 在σ_opt附近运行的能力 定义智能度： ι = I_eff(σ) / I_eff(σ_opt) = I_eff(σ) / I_eff_max ι ∈ \[0, 1\] ι = 1：有效信息最大，最优智能 ι → 0：有效信息趋零，无智能](../../../%E7%BB%9F%E4%B8%80%E5%87%BD%E6%95%B0%E6%80%BB%E8%A1%A8/0274-D238-%E6%99%BA%E8%83%BD%E7%9A%84%E9%97%A8%E6%8E%A7%E7%B2%BE%E5%BA%A6%E6%9C%80%E4%BC%98%E5%AE%9A%E7%90%86.md)
  - 类型/状态：`FUNCTION_ASSET` · `QUARANTINE_UNTIL_DEFINED`
  ...
  - 来源：`统一函数总表/0274-D238-智能的门控精度最优定理.md`
  - 依赖：—；被引用：—
- [中文：案例说明：C_exit(geo)=κ×ρ^(-α)×r^β×τ](../../../%E7%BB%9F%E4%B8%80%E6%A1%88%E4%BE%8B%E6%80%BB%E8%A1%A8/0146-C-146-%E8%A5%BF%E5%AE%89%E5%8A%A0%E6%B2%B9%E7%AB%99%E4%BE%BF%E5%88%A9%E5%BA%97.md)
  - 类型/状态：`FUNCTION_ASSET` · `QUARANTINE_UNTIL_DEFINED`
  - 可搜索名称：IMPLICIT-80C4CA8ED525C020 / 中文：案例说明：C_exit(geo)=κ×ρ^(-α)×r^β×τ / IMPLICIT-80C4CA8ED525C020
  - 来源：`统一案例总表/0146-C-146-西安加油站便利店.md`
  - 依赖：—；被引用：—
- [中文：案例说明：P(发现)=P(L₅激活)×P(L₅不退化)](../../../%E7%BB%9F%E4%B8%80%E6%A1%88%E4%BE%8B%E6%80%BB%E8%A1%A8/0137-C-137-%E6%A1%86%E6%9E%B6%E5%8F%91%E7%8E%B0%E8%BF%87%E7%A8%8B.md)
  - 类型/状态：`FUNCTION_ASSET` · `QUARANTINE_UNTIL_DEFINED`
  ...
  - 依赖：—；被引用：—
- [中文：案例说明：P(发现)=P(L₅激活)×P(L₅不退化)](../../../%E7%BB%9F%E4%B8%80%E6%A1%88%E4%BE%8B%E6%80%BB%E8%A1%A8/0127-C-127-%E6%A1%86%E6%9E%B6%E5%8F%91%E7%8E%B0%E8%BF%87%E7%A8%8B.md)
  - 类型/状态：`FUNCTION_ASSET` · `QUARANTINE_UNTIL_DEFINED`
